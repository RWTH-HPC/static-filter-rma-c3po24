...
/*
{
    ...
    "RACE_PAIR": ["MPI_Accumulate@58","MPI_Accumulate@63"],
    "NPROCS": 3,
    "CONSISTENCY_CALLS": ["MPI_Win_fence"],
    "SYNC_CALLS": ["MPI_Barrier", "MPI_Win_fence"],
    "DESCRIPTION": "Two atomic operations acc and acc. Since the displacement is
1 byte, there are no atomicity guarantees."
}
*/
// RACE LABELS END
...
int main(int argc, char **argv) {
  ...
  MPI_Win_allocate(WIN_SIZE * sizeof(int), 1, MPI_INFO_NULL, MPI_COMM_WORLD,
                   &win_base, &win);
  ...
  if (rank == 2) {
    ...
    // CONFLICT
    MPI_Accumulate(value2, 4, MPI_INT, 1, 1, 4, MPI_INT, MPI_SUM, win);
  }
  MPI_Win_fence(0, win);
