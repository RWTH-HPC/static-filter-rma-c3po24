...
{
    "RACE_KIND": "none",
    "ACCESS_SET": ["local buffer read","local buffer read"],
    "NPROCS": 2,
    "DESCRIPTION": "Two non-conflicting operations put and put executed
concurrently with no race."
}
*/
// RACE LABELS END
// RACE_KIND: none
...
__attribute__((noinline)) void deeeeeeeep(int *buf, MPI_Win win) {
  deeeeeeeeep(buf, win);
}
__attribute__((noinline)) void deeeeeeep(int *buf, MPI_Win win) {
  deeeeeeeep(buf, win);
}
__attribute__((noinline)) void deeeeeep(int *buf, MPI_Win win) {
  deeeeeeep(buf, win);
}
__attribute__((noinline)) void deeeeep(int *buf, MPI_Win win) {
  deeeeeep(buf, win);
}
__attribute__((noinline)) void deeeep(int *buf, MPI_Win win) {
  deeeeep(buf, win);
}
__attribute__((noinline)) void deeep(int *buf, MPI_Win win) {
  deeeep(buf, win);
}
