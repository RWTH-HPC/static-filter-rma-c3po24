...
/*
{
    ...
    "RACE_PAIR": ["MPI_Fetch_and_op@56","LOAD@58"],
    "NPROCS": 2,
    "DESCRIPTION": "Two conflicting operations fop and load executed
...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    // CONFLICT
    MPI_Fetch_and_op(&value2, &value, MPI_INT, 1, 0, MPI_SUM, win);
    // CONFLICT
    printf("value is %d\n", *buf);
  }
  MPI_Win_fence(0, win);
