...
/*
{
    ...
    "RACE_PAIR": ["MPI_Put@69","LOAD@86"],
    "CONSISTENCY_CALLS": ["MPI_Win_lock","MPI_Win_unlock"],
    "SYNC_CALLS": ["MPI_Barrier"],
    "NPROCS": 2,
    "DESCRIPTION": "Two conflicting operations MPI_Get and a local load with
missing synchronization, because only the other thread at the origin
synchronizes with the target."
}
*/
// RACE LABELS END
...
#pragma omp section
        {
          ...
          // CONFLICT
          MPI_Put(&value, 1, MPI_INT, 1, 0, 1, MPI_INT,
                  win); // Put on win_base[0] at process 1
