...
/*
{
    ...
    "RACE_PAIR": ["MPI_Get@86","LOAD@94"],
    "CONSISTENCY_CALLS": ["MPI_Win_lock","MPI_Win_unlock"],
    "SYNC_CALLS": ["MPI_Barrier"],
    "NPROCS": 2,
    "DESCRIPTION": "Two conflicting operations MPI_Get and a local load with
missing synchronization at process 0. Since there is no taskwait, local load and
access epoch of the MPI_Get are concurrent, resulting in a local buffer race."
}
*/
// RACE LABELS END
...
#pragma omp task shared(flag)
        ...
        // make execution of task on another thread probable by waiting for
        // signal of task
        my_wait(&flag, 1);
        // CONFLICT
