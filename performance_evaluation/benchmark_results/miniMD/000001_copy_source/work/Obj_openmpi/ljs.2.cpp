...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i < argc; i++) {
    ...
    if((strcmp(argv[i], "-f") == 0) || (strcmp(argv[i], "--data_file") == 0)) {
      if(in.datafile == __null) in.datafile = new char[1000];
    ...
    if((strcmp(argv[i], "-h") == 0) || (strcmp(argv[i], "--help") == 0)) {
      printf("\n-----------------------------------------------------------------------------------------------------------\n");
      printf("-------------" "miniMD-Reference 2.0 (MPI+OpenMP)" "--------------------\n");
      printf("-------------------------------------------------------------------------------------------------------------\n\n");
  ...
  if(neighbor_size < 0 && in.datafile == __null) {
  ...
  if(me == 0) {
    fprintf(stdout, "# " "miniMD-Reference 2.0 (MPI+OpenMP)" " output ...\n");
...
#pragma omp parallel
  ...
  if(me == 0) {
    ...
    printf("%i %i %i %i %lf %lf %lf %lf %lf %lf %lf PERF_SUMMARY %lf\n\n\n",
           ...
           timer.array[0], timer.array[2], timer.array[3], timer.array[1], time_other,
           1.0 * natoms * integrate.ntimes / timer.array[0], 1.0 * natoms * integrate.ntimes / timer.array[0] / nprocs / num_threads, timer.array[4]);
