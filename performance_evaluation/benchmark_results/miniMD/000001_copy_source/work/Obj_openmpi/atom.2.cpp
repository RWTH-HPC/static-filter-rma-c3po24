...
Atom::Atom(int ntypes_)
{
  ...
  x = v = f = xold = x_copy = v_copy = __null;
  type = type_copy = __null;
  comm_size = 3;
  reverse_size = 3;
  border_size = 4;
...
void Atom::growarray()
{
  ...
  if(x == __null || v == __null || f == __null || xold == __null) {
...
#pragma omp for
  for(int i = 0; i < nlocal; i++) {
    if(x[i*3 + 0] < 0.0) x[i * 3 + 0] += box.xprd;

    if(x[i * 3 + 0] >= box.xprd) x[i * 3 + 0] -= box.xprd;

    if(x[i * 3 + 1] < 0.0) x[i * 3 + 1] += box.yprd;

    if(x[i * 3 + 1] >= box.yprd) x[i * 3 + 1] -= box.yprd;

    if(x[i * 3 + 2] < 0.0) x[i * 3 + 2] += box.zprd;

    if(x[i * 3 + 2] >= box.zprd) x[i * 3 + 2] -= box.zprd;
  }
}
...
#pragma omp for schedule(static)
    for(i = 0; i < n; i++) {
      j = list[i];
      buf[3 * i] = x[j * 3 + 0] + pbc_flags[1] * box.xprd;
      buf[3 * i + 1] = x[j * 3 + 1] + pbc_flags[2] * box.yprd;
      buf[3 * i + 2] = x[j * 3 + 2] + pbc_flags[3] * box.zprd;
    }
  }
}
...
#pragma omp for schedule(static)
  for(i = 0; i < n; i++) {
    x[(first + i) * 3 + 0] = buf[3 * i];
    x[(first + i) * 3 + 1] = buf[3 * i + 1];
    x[(first + i) * 3 + 2] = buf[3 * i + 2];
  }
}
...
#pragma omp for schedule(static)
  for(i = 0; i < n; i++) {
    buf[3 * i] = f[(first + i) * 3 + 0];
    buf[3 * i + 1] = f[(first + i) * 3 + 1];
    buf[3 * i + 2] = f[(first + i) * 3 + 2];
  }
}
...
int Atom::pack_border(int i, MMD_float* buf, int* pbc_flags)
{
  ...
  if(pbc_flags[0] == 0) {
    ...
    buf[m++] = x[i * 3 + 0] + pbc_flags[1] * box.xprd;
    buf[m++] = x[i * 3 + 1] + pbc_flags[2] * box.yprd;
    buf[m++] = x[i * 3 + 2] + pbc_flags[3] * box.zprd;
    buf[m++] = type[i];
  }
   ...
   binpos[i] += binpos[i-1];
    if(copy_size<nmax) {
     ...
     destroy_1d_int_array(type_copy);
      x_copy = (MMD_float*) create_2d_MMD_float_array(nmax, 3);
      v_copy = (MMD_float*) create_2d_MMD_float_array(nmax, 3);
...
#pragma omp for
  for(int mybin = 0; mybin<mbins; mybin++) {
   ...
   new_v[new_i*3 +0] = old_v[old_i*3 +0];
   new_v[new_i*3 +1] = old_v[old_i*3 +1];
   new_v[new_i*3 +2] = old_v[old_i*3 +2];
