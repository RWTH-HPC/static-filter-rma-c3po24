...
P2POp::P2POp(
    ...
    : myMatcher(matcher), myIsSend(isSend), myTag(tag), myToRank(toRank), wasWcReceive(false),
      myWcRecvFirstMatchingWorldRank(-1), myHasRequest(false), myRequest(0), myComm(comm),
      myType(datatype), myCount(count), myPId(pId), myLId(lId), myIsInSusendedWcOpQueue(false),
      mySendMode(mode)
{
...
P2POp::P2POp(
    ...
    MustSendMode mode)
    : myMatcher(matcher), myIsSend(isSend), myTag(tag), myToRank(toRank), wasWcReceive(false),
      myWcRecvFirstMatchingWorldRank(-1), myHasRequest(true), myRequest(request), myComm(comm),
      myType(datatype), myCount(count), myPId(pId), myLId(lId), myIsInSusendedWcOpQueue(false),
      mySendMode(mode)
{
    myRank = myMatcher->myPIdMod->getInfoForId(myPId).rank;
...
{
    if (!myIsSend && myToRank == myMatcher->myConsts->getAnySource() && !myIsInSusendedWcOpQueue) {
...
void P2POp::logAsLost(int rank)
{
    ...
    stream << "The application fails to match a point-to-point operation before it issues "
              "MPI_Finalize. "
           << "The outstanding " << type << " point-to-point message of rank " << rank
           << " needs to be "
    ...
    stream << " (both as ranks in MPI_COMM_WORLD). The outstanding " << type
    ...
    stream << "). A correct application must match all point-to-point communications before "
...
// getToRank
//=============================
int P2POp::getToRank(void) { return myToRank; }

//=============================
// getComm
//=============================
I_Comm* P2POp::getComm(void) { return myComm; }
...
// getPersistentComm
//=============================
I_CommPersistent* P2POp::getPersistentComm(void) { return myComm; }

//=============================
// getComm
//=============================
int P2POp::getIssuerRank(void) { return myRank; }
...
// hasRequest
//=============================
bool P2POp::hasRequest(void) { return myHasRequest; }

//=============================
// getRequest
//=============================
MustRequestType P2POp::getRequest(void) { return myRequest; }

//=============================
// updateToSource
//=============================
void P2POp::updateToSource(int newToRank) { myToRank = newToRank; }
...
// getTag
//=============================
int P2POp::getTag(void) { return myTag; }

//=============================
// getSendMode
//=============================
MustSendMode P2POp::getSendMode(void) { return mySendMode; }

//=============================
// wasIssuedAsWcReceive
//=============================
bool P2POp::wasIssuedAsWcReceive(void) { return wasWcReceive; }

//=============================
// isSend
//=============================
bool P2POp::isSend(void) { return myIsSend; }

//=============================
// getPId
//=============================
MustParallelId P2POp::getPId(void) { return myPId; }

//=============================
// getLId
//=============================
MustLocationId P2POp::getLId(void) { return myLId; }

//=============================
// copy
//=============================
P2POp* P2POp::copy(void) { return new P2POp(this); }
...
// setFirstWorldRankWithValidMatch
//=============================
int P2POp::getFirstWorldRankWithValidMatch(void) { return myWcRecvFirstMatchingWorldRank; }

#ifdef DOT
//=============================
// generateTypemismatchHtml
//=============================
void P2POp::generateTypemismatchHtml(std::string const& base_file_name_without_ext)
{
