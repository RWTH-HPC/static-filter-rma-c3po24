...
! SPDX-License-Identifier: BSD-3-Clause
      ...
      PROGRAM simple
! RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/fortran_waitall 2>&1 \
! RUN: | %filecheck --implicit-check-not \
! RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

! RUN: %must-run --must:language fortran %mpiexec-numproc-flag 2 %must-bin-dir/fortran_waitall 2>&1 \
! RUN: | %filecheck --implicit-check-not \
! RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s
