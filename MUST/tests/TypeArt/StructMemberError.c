...
int main(int argc, char** argv)
{
    ...
    if (rank == 0) {
        // CHECK: Incompatible buffer of type 2 (int32) - expected MPI_FLOAT
        MPI_Send(&s1.b, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD);
    } else if (rank == 1) {
        // CHECK: Incompatible buffer of type 2 (int32) - expected MPI_FLOAT
        MPI_Recv(&s1.b, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
