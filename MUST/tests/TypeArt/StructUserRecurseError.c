...
int main(int argc, char** argv)
{
    ...
    if (rank == 0) {
        // CHECK: Incompatible buffer of type 5 (float) - expected MPI_DOUBLE
        MPI_Send(&s1.a, 2, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
    } else if (rank == 1) {
        // CHECK: Incompatible buffer of type 5 (float) - expected MPI_DOUBLE
        MPI_Recv(&s1.a, 2, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
