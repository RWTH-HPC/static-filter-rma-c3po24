...
int main(int argc, char** argv)
{
    ...
    MPI_Datatype types[3] = {MPI_DOUBLE, MPI_INT, MPI_FLOAT}; // FIX: Last type should be MPI_DOUBLE
    ...
    if (rank == 0) {
        // CHECK: Incompatible buffer element of type 6 (double) of struct struct.S1
