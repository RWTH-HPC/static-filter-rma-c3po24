...
// RUN: %must-run %mpiexec-numproc-flag 3 %must-bin-dir/waitanyNoDl 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

// RUN: %must-run-ddl %mpiexec-numproc-flag 3 %must-bin-dir/DDlwaitanyNoDl \
// RUN: 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s
...
/**
 * @file waitanyNoDl.c
 * Simple test with an MPI_Waitany call causes no deadlock (No Error).
 *
 * Description:
