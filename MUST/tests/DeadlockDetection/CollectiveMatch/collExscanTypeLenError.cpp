...
// RUN: %must-bin-dir/collExscanTypeLenError 2>&1 \
// RUN: | %filecheck-may-segfault --check-prefix 'CHECK-LOCAL' %s

// RUN: %must-run %mpiexec-numproc-flag 3 --must:layout \
// RUN: %builddir/tests/DeadlockDetection/DCollectiveMatch/collExscanTypeLenErrorlayout.xml \
// RUN: %must-bin-dir/DcollExscanTypeLenError 2>&1 \
...
/**
 * @file collExscanTypeLenError.cpp
...
int main(int argc, char** argv)
{
    ...
    MPI_Exscan(outbuf, inbuf, 18 + rank % 2, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
