...
// RUN: %must-bin-dir/collRootMissmatchErrorGather 2>&1 \
// RUN: | %filecheck --check-prefix 'CHECK-LOCAL' %s

// RUN: %must-run %mpiexec-numproc-flag 2 --must:layout \
// RUN: %builddir/tests/DeadlockDetection/DCollectiveMatch/collRootMissmatchErrorGatherlayout.xml \
// RUN: %must-bin-dir/DcollRootMissmatchErrorGather 2>&1 \
...
/**
 * @file collRootMissmatchErrorGather.c
 * A test with root mismatch (Error).
 *
 * Description:
 * All processes execute a gather with a root of 0, except for rank 1 which uses 1 as its root (ERROR).
 *
...
{
    int rank, size, temp, root, *tempArray;
    ...
    MPI_Gather(&temp, 1, MPI_INT, tempArray, 1, MPI_INT, root, MPI_COMM_WORLD);
