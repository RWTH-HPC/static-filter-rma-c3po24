...
// RUN: %must-bin-dir/collAllgatherTypeLenError 2>&1 \
// RUN: | %filecheck-may-segfault --check-prefix 'CHECK-LOCAL' %s

// RUN: %must-run %mpiexec-numproc-flag 3 --must:layout \
// RUN: %builddir/tests/DeadlockDetection/DCollectiveMatch/collAllgatherTypeLenErrorlayout.xml \
// RUN: %must-bin-dir/DcollAllgatherTypeLenError 2>&1 \
...
/**
 * @file collAllgatherTypeLenError.cpp
 * Test for collective matching (with error).
 *
 * Description:
 * Performs a MPI_Allgather collective with an error
 *
 * ERROR: send {6,6,6} INTs, recv {18,21,18} INTs (works with OpenMPI!)
 *
 *  @date 23.03.2012
