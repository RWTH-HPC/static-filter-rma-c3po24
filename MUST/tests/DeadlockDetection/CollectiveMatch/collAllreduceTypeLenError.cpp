...
// RUN: %must-bin-dir/collAllreduceTypeLenError 2>&1 \
// RUN: | %filecheck-may-segfault --check-prefix 'CHECK-LOCAL' %s

// RUN: %must-run %mpiexec-numproc-flag 3 --must:layout \
// RUN: %builddir/tests/DeadlockDetection/DCollectiveMatch/collAllreduceTypeLenErrorlayout.xml \
// RUN: %must-bin-dir/DcollAllreduceTypeLenError 2>&1 \
...
/**
 * @file collAllreduceTypeLenError.cpp
 * Collective matching test.
 *
 * Description:
 * Performs a MPI_Allreduce collective with an error
 *
 *
 *  @date 27.03.2012
...
int main(int argc, char** argv)
{
    ...
    MPI_Allreduce(outbuf, inbuf, 18 + rank % 2, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
