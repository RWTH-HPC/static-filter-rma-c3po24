...
// RUN: %must-bin-dir/DnbcCollAlltoallvTypeMatchError 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*Two collective calls cause a type mismatch! This call sends data to the call in reference 1}}

/**
 * @file nbcCollAlltoallvTypeMatchError.c
 * A test with an incorrect MPI_Ialltoallv call (Error).
 *
 * Description:
