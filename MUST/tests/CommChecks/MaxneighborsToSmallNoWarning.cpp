...
// RUN: %must-bin-dir/MaxneighborsToSmallNoWarning 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

/**
 * @file MaxneighborsToSmallNoWarning.cpp
 ...
 * on rank 3 without any errors or warnings.
 *
 *
 *  @date 29.04.2011
...
int main(int argc, char** argv)
{
    ...
    if (rank == 3) {
        MPI_Graph_neighbors(comm1, 3, 2, count);
    }
