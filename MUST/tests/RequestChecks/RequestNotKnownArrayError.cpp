...
// RUN: %must-bin-dir/RequestNotKnownArrayError 2>&1 \
// RUN: | %filecheck-may-segfault %s

// CHECK: [MUST-REPORT]{{.*Error:.*Argument.*[(]array_of_requests[)].*the following entries are unknown requests:}}
...
int main(int argc, char** argv)
{
    ...
    MPI_Status status[4];
    MPI_Request request
        [4]; /** 3 of 4 requests are used by Irecv, request[3] is unknown when MPI_Waitall is called **/
    //Enough tasks ?
    if (size < 2) {
