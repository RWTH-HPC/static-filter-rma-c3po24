...
// RUN: %must-bin-dir/IntegerNotOneOrZeroArrayWarning 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*Warning:.*The array of logical arguments.*[(]periods[)].*has entries that are neither 1 or 0, which is valid but}}
