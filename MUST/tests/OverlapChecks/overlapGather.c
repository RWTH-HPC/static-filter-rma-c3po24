...
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/OverlapGather 2>&1 \
// RUN: | %filecheck %s

// CHECK-DAG: [MUST-REPORT] Error: from: call MPI_Gather@0: The memory regions to be transfered by this receive operation overlap with regions spanned by a pending non-blocking operation!
// CHECK-DAG: [MUST-REPORT]  Representative location:
// CHECK-DAG: Reference 1: call MPI_Isend@rank 0

// CHECK-DAG: [MUST-REPORT] Error: from: call MPI_Gather@1: The memory regions to be transfered by this send operation overlap with regions spanned by a pending non-blocking receive operation!
// CHECK-DAG: [MUST-REPORT]  Representative location:
 ...
 * A must overlap test.
 * This test let a MPI_Gather overlap with pending communication requests.
 *
...
int main(int argc, char** argv)
{
    ...
    MPI_Gather(inbuf, 1, structtype, inbuf + 1, 1, structtype, 0, MPI_COMM_WORLD);
