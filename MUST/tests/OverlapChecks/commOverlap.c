...
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/CommOverlap 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*Error. from. call MPI_Irecv@.*1. The memory regions to be transfered by this receive operation overlap with regions spanned by a pending non-blocking operation!}}

/**
 * @file commOverlap.c
 * A must overlap test.
 * Creates overlapping requests
 *
