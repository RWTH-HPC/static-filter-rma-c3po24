...
// RUN: %must-bin-dir/CommOverlapInitWaitall 2>&1 \
// RUN: | %filecheck %s
...
/**
 * @file commOverlapInitWaitall.c
 * A must overlap test.
 * Creates overlapping persistent requests and finishes some in between
 *
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0) {
        for (i = 0; i < COUNT; i++) {
            ...
            MPI_Startall(i + 1, request);
            MPI_Waitall(i + 1, request, status);
        }
        for (i = 0; i < COUNT; i++) {
