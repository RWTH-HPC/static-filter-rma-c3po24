...
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/OverlapAllreduce \
// RUN: 2>&1 \
// RUN: | %filecheck %s

// CHECK-DAG: [MUST-REPORT] Error: from: call MPI_Allreduce@1: The memory regions to be transfered by this send operation overlap with regions spanned by a pending non-blocking receive operation!
// CHECK-DAG: [MUST-REPORT] {{.*}}References
// CHECK-DAG: Reference 1: call MPI_Irecv@rank 1
// CHECK-DAG: [MUST-REPORT] Error: from: call MPI_Allreduce@1: The memory regions to be transfered by this receive operation overlap with regions spanned by a pending non-blocking operation!
// CHECK-DAG: [MUST-REPORT] {{.*}}References
// CHECK-DAG: Reference 1: call MPI_Irecv@rank 1
