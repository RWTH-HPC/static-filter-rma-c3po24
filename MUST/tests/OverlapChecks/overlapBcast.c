...
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/OverlapBcast 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*Error[:] from[:] call MPI_Bcast@.*1[:] The memory regions to be transfered by this receive operation overlap with regions spanned by a pending non-blocking operation!}}
...
int main(int argc, char** argv)
{
    ...
    MPI_Bcast(inbuf, 1, structtype, 0, MPI_COMM_WORLD);
