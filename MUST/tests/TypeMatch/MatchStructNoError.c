...
// RUN: %builddir/tests/TypeMatch/MatchStructNoErrorlayout.xml \
// RUN: %must-bin-dir/MatchStructNoError 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

// RUN: %must-run %mpiexec-numproc-flag 2 --must:layout \
// RUN: %builddir/tests/TypeMatch/DMatchStructNoErrorlayout.xml \
// RUN: %must-bin-dir/DMatchStructNoError 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

/**
 * @file MatchStructNoError.c
