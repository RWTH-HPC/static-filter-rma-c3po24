...
// RUN: %builddir/tests/TypeMatch/MatchMultipleContiMissmatchErrorlayout.xml \
// RUN: %must-bin-dir/MatchMultipleContiMissmatchError 2>&1

// RUN: %must-run %mpiexec-numproc-flag 2 --must:layout \
// RUN: %builddir/tests/TypeMatch/DMatchMultipleContiMissmatchErrorlayout.xml \
// RUN: %must-bin-dir/DMatchMultipleContiMissmatchError 2>&1
...
int main(int argc, char** argv)
{
    ...
    if (rank == 1) {
        //Create a conti type
        MPI_Type_contiguous(10, MPI_UNSIGNED_CHAR, &conti);
        MPI_Type_commit(&conti);
