...
// RUN: %builddir/tests/TypeMatch/MatchIndexedBlockNoErrorlayout.xml \
// RUN: %must-bin-dir/MatchIndexedBlockNoError 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

// RUN: %must-run %mpiexec-numproc-flag 2 --must:layout \
// RUN: %builddir/tests/TypeMatch/DMatchIndexedBlockNoErrorlayout.xml \
// RUN: %must-bin-dir/DMatchIndexedBlockNoError 2>&1 \
// RUN: | %filecheck --implicit-check-not \
// RUN: '[MUST-REPORT]{{.*(Error|ERROR|Warning|WARNING)}}' %s

/**
 * @file MatchIndexedBlockNoError.c
...
int main(int argc, char** argv)
{
    ...
    //Create a conti type
    MPI_Type_create_indexed_block(3, 3, displacements, MPI_LONG, &newType);
