...
// RUN: %builddir/tests/TypeMatch/MatchVectorLengthErrorlayout.xml \
// RUN: %must-bin-dir/MatchVectorLengthError 2>&1 \
// RUN: | %filecheck-may-segfault --check-prefix 'CHECK-LOCAL' %s

// RUN: %must-run %mpiexec-numproc-flag 2 --must:layout \
// RUN: %builddir/tests/TypeMatch/DMatchVectorLengthErrorlayout.xml \
// RUN: %must-bin-dir/DMatchVectorLengthError 2>&1 \
...
/**
 * @file MatchVectorLengthError.c
 * Type matching test with an error.
 *
 * Description:
 * A single send-recv match, the send and reveice both use a vector type,
 * the send sends too many basic types (ERROR).
 *
...
int main(int argc, char** argv)
{
    ...
    MPI_Type_vector(3, blocklength, 6, MPI_LONG, &newType);
