    ...
    *dthetamax = 0.0;
    ...
    for (y = mympi->start_y; y < mympi->start_y + mympi->num_cells_y; y++) {
        for (x = mympi->start_x; x < mympi->start_x + mympi->num_cells_x; x++) {
            dtheta = (grid->theta[y][x - 1] + grid->theta[y][x + 1] - 2 * grid->theta[y][x]) /
                         (grid->dx * grid->dx) +
                     (grid->theta[y - 1][x] + grid->theta[y + 1][x] - 2 * grid->theta[y][x]) /
                         (grid->dy * grid->dy);
    ...
    usleep(1000); //Very important, removing this can cause deadlock ;)
    if (mympi->rank != 0) {
        MPI_Status status;
...
void heatBoundary(heatGrid* grid, dataMPI* mympi)
{
    ...
    MPI_Isend(
        &(grid->theta[mympi->start_y][mympi->start_x]),
        mympi->num_cells_x + 1,
        MPI_DOUBLE,
        mympi->up,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_y + mympi->num_cells_y][mympi->start_x]),
        mympi->num_cells_x + 1,
        MPI_DOUBLE,
        mympi->down,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_y - 1][mympi->start_x]),
        mympi->num_cells_x + 1,
        MPI_DOUBLE,
        mympi->up,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_y][mympi->start_x + mympi->num_cells_x]),
        1,
        mympi->columntype,
        mympi->right,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_y][mympi->start_x - 1]),
        1,
        mympi->columntype,
        mympi->left,
        123,
...
void heatMPIGather(heatGrid* grid, dataMPI* mympi)
{
    ...
    /*Master Receives data*/
    {
        ...
        for (i = 1; i < size; i++) {
            ...
            MPI_Recv(
                &grid->theta[block_size[1]][block_size[0]],
                1,
                blocktype,
                i,
                123,
