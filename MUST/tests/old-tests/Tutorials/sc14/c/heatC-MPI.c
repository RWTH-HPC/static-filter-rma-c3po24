 ...
 ******************************************************/
double heatInitFunc(double x) { return (-4.0 * x * x * x + 4.0 * x * x - 1.0 * x + 1.0); }
    ...
    *dthetamax = 0.0;
    ...
    for (x = mympi->start_x; x < mympi->start_x + mympi->num_cells_x; x++) {
        for (y = mympi->start_y; y < mympi->start_y + mympi->num_cells_y; y++) {
            dtheta = (grid->theta[x - 1][y] + grid->theta[x + 1][y] - 2 * grid->theta[x][y]) /
                         (grid->dx * grid->dx) +
                     (grid->theta[x][y - 1] + grid->theta[x][y + 1] - 2 * grid->theta[x][y]) /
                         (grid->dy * grid->dy);
...
void heatBoundary(heatGrid* grid, dataMPI* mympi)
{
    ...
    MPI_Isend(
        &(grid->theta[mympi->start_x][mympi->start_y]),
        mympi->num_cells_y,
        MPI_DOUBLE,
        mympi->left,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_x + mympi->num_cells_x][mympi->start_y]),
        mympi->num_cells_y,
        MPI_DOUBLE,
        mympi->right,
        123,
    ...
    MPI_Isend(
        &(grid->theta[mympi->start_x + mympi->num_cells_x - 1][mympi->start_y]),
        mympi->num_cells_y,
        MPI_DOUBLE,
        mympi->right,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_x - 1][mympi->start_y]),
        mympi->num_cells_y,
        MPI_DOUBLE,
        mympi->left,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_x][mympi->start_y + mympi->num_cells_y]),
        1,
        mympi->rowtype,
        mympi->down,
        123,
    ...
    MPI_Isend(
        &(grid->theta[mympi->start_x][mympi->start_y + mympi->num_cells_y - 1]),
        1,
        mympi->rowtype,
        mympi->down,
        123,
    ...
    MPI_Recv(
        &(grid->theta[mympi->start_x][mympi->start_y - 1]),
        1,
        mympi->rowtype,
        mympi->up,
        123,
...
void heatMPIGather(heatGrid* grid, dataMPI* mympi)
{
    ...
    /*Master Receives data*/
    {
        ...
        for (i = 1; i < size; i++) {
            ...
            MPI_Recv(
                &grid->theta[block_size[0]][block_size[1]],
                1,
                blocktype,
                i,
                123,
