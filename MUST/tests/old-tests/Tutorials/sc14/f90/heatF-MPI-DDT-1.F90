...
contains
  ...
  subroutine heatAllocate(grid, xsize, ysize)
    ...
    grid%xsize = xsize
    grid%ysize = ysize
    allocate( grid%theta   ( 0:xsize, 0:ysize ) )
    allocate( grid%thetanew( 0:xsize, 0:ysize ) )
    grid%theta    = 0.0d0
    grid%thetanew = 0.0d0
    grid%dx = 1.0d0
    grid%dy = 1.0d0
  ...
  subroutine heatBoundary(grid, mympi)
    ...
    CALL MPI_ISEND (grid%theta(mympi%start_x,mympi%start_y), &
            mympi%num_cells_x, MPI_DOUBLE_PRECISION, mympi%up, 123, mympi%cart, requests(3), err)
    ...
    CALL MPI_ISEND (grid%theta(mympi%start_x, mympi%start_y+mympi%num_cells_y-1), &
            mympi%num_cells_x, MPI_DOUBLE_PRECISION, mympi%down, 123, mympi%cart, requests(4), err)
    ...
    ! Complete the non-blocking receives
	CALL MPI_WAITALL (4, requests, statuses, err)
  ...
  subroutine heatMPISetup (grid, configMPI)
    ...
    integer :: size, err, dims(2)=(/0,0/), coords(2)
