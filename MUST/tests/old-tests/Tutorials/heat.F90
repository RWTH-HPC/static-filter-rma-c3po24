...
!   and creates output for gnuplot.
!
!   March 2009
...
contains
  ...
  subroutine heatTimestep(grid, mympi, dt, dthetamax)
    ...
    do y=mympi%start_y,mympi%start_y + mympi%num_cells_y -1
      do x=mympi%start_x,mympi%start_x + mympi%num_cells_x -1
        ...
        mymax = max(abs(dtheta), mymax) ! save max theta for the exit condition
  ...
  subroutine heatBoundary(grid, mympi)
    ...
    CALL MPI_ISEND (grid%theta(mympi%start_x, mympi%start_y), &
            1, mympi%columntype, mympi%left, 123, mympi%cart, requests(1), err)
    ...
    CALL MPI_ISEND (grid%theta(mympi%start_x+mympi%num_cells_x-1, mympi%start_y), &
            1, mympi%columntype, mympi%right, 123, mympi%cart, requests(2), err)
    ...
    CALL MPI_ISEND (grid%theta(mympi%start_x,mympi%start_y), &
            mympi%num_cells_x+1, MPI_DOUBLE_PRECISION, mympi%up, 123, mympi%cart, &
            requests(3), err)
    ! Receive lower border row from bottom neighbor
    CALL MPI_RECV  (grid%theta(mympi%start_x, mympi%start_y+mympi%num_cells_y), &
            mympi%num_cells_x+1, MPI_DOUBLE_PRECISION, mympi%down, 123, mympi%cart, status, err)

    ! Send lower row to bottom neighbor
    CALL MPI_ISEND (grid%theta(mympi%start_x, mympi%start_y+mympi%num_cells_y-1), &
            mympi%num_cells_x+1, MPI_DOUBLE_PRECISION, mympi%down, 123, mympi%cart, &
            requests(3), err)
    ! Receive upper border row from top neighbor
    CALL MPI_RECV  (grid%theta(mympi%start_x, mympi%start_y-1), &
            mympi%num_cells_x+1, MPI_DOUBLE_PRECISION, mympi%up, 123, mympi%cart, status, err)

    CALL MPI_WAITALL (3, requests, status, err)
  ...
  subroutine heatMPISetup (grid, configMPI)
    ...
    CALL MPI_CART_CREATE (MPI_COMM_WORLD, 2, dims, periods, 0, configMPI%cart, err)
...
program heatExample
  ...
  integer :: step, noutput

  ! create heatGrid and initialize variables
  call heatAllocate(mygrid, 20, 20)
  ...
  ! time stepping loop
  do while (dthetamax > 0.4)
    step = step + 1
    call heatBoundary(mygrid, mympi)
    call heatTimestep(mygrid, mympi, dt, dthetamax )
    ! do some output depending on dthetamax
    if( (dthetamax < 0.1  .and. noutput==1) .or. &
        (dthetamax < 0.02 .and. noutput==2)         ) then
  ...
  if (mympi%rank .EQ. 0) then
    ...
    write (*,*)
    write (*,*) 'grid after ',step,' iterations:'
    call heatPrint(mygrid)
    ...
    write (*,'(A,F10.3)') ' initial Energy: ',energyInitial
    write (*,'(A,F10.3)') '   final Energy: ',energyFinal
    write (*,'(A,F10.3)') '     Difference: ',energyFinal-energyInitial
  end if
