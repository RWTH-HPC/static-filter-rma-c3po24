...
/**
 * @file alltoallvs.c
 * A stress test for Collective checking.
 *
...
{
    int rank, size, i, *sbuf, *rbuf, count = 1, *scounts, *sdispls, *rcounts, *rdispls;
    ...
    if (rank == 0)
        printf("alltoallvs@%d rounds=%lld count=%d\n", size, numColls, count);

    rbuf = (int*)malloc(sizeof(int) * size * count);
    sbuf = (int*)malloc(sizeof(int) * size * count);
    ...
    for (i = 0; i < size; i++) {
        ...
        rcounts[i] = count;
        rdispls[i] = i * count;
        sdispls[i] = (count * (size - 1)) - i * count;
    }
