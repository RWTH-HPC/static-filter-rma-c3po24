/* -*- Mode: C; -*- */
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/umpire_lost-request2 \
// RUN: 2>&1 \
// RUN: | %filecheck %s
...
int main(int argc, char** argv)
{
    ...
    for (cycle = 0; cycle < CYCLE_COUNT; cycle++) {
        ...
        if (rank < 2)
            printf("Split_comm handle %s in cycle %i\n", "split_comm", cycle);
        ...
        if (split_rank == 0) {
            ...
            rc = MPI_Irecv(&recv_int, 1, MPI_INT, 0, 0, split_comm, &request);
            if (rc != MPI_SUCCESS) {
