...
// RUN: %must-bin-dir/umpire_intercomm_create-deadlock2 2>&1 \
// RUN: | %filecheck %s
...
int main(int argc, char** argv)
{
    ...
    if (nprocs < 3) {
        ...
        if (temp != MPI_COMM_NULL) {
            ...
            if ((trank == 0) && (rank % 2)) {
                MPI_Recv(buf0, buf_size, MPI_INT, rleader, 0, MPI_COMM_WORLD, &status);
            }
            ...
            if (intercomm != MPI_COMM_NULL) {
                ...
                if (rnprocs > drank) {
                    ...
                    printf(
                        "(%d) Intercomm too small (lrank = %d; remote size = %d)\n",
                        rank,
                        drank,
                        rnprocs);
                }
