...
// RUN: %must-bin-dir/umpire_pt2pt-byte-int-mismatch 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*Error: from: call MPI_Irsend@.*0: A send and a receive operation use datatypes that do not match!}}
...
int main(int argc, char** argv)
{
    ...
    MPI_Buffer_attach(bbuf, sizeof(int) * (BUF_SIZE + MPI_BSEND_OVERHEAD) * 2 * NUM_BSEND_TYPES);
    ...
    if (rank == 0) {
        ...
        MPI_Send_init(
            &buf[(send_t_number * 2 + 1) * BUF_SIZE],
            BUF_SIZE,
            MPI_INT,
            1,
            send_t_number * 2 + 1,
    ...
    for (m = 0; m < NUM_RECV_TYPES; m++) {
        if ((m == 1) && (rank == 1)) {
            ...
            for (j = 0; j < NUM_SEND_TYPES * 2; j += 2) {
                MPI_Recv_init(&buf[j * BUF_SIZE], BUF_SIZE, MPI_INT, 0, j, comm, &aReq[j]);
                MPI_Recv_init(
                    &buf[(j + 1) * BUF_SIZE],
                    BUF_SIZE * sizeof(int),
                    MPI_BYTE,
                ...
                if (rank == 0) {
                    ...
                    case 7:
                        ...
                        while (total < NUM_SEND_TYPES * 2) {
                            ...
                            while (!outcount) {
                                MPI_Testsome(NUM_SEND_TYPES * 2, aReq, &outcount, indices, aStatus);
                            }
    ...
    if (rank == 0) {
        ...
        for (i = 2 * (NUM_SEND_TYPES - NUM_PERSISTENT_SEND_TYPES); i < 2 * NUM_SEND_TYPES; i++) {
    ...
    assert(at_size == sizeof(int) * (BUF_SIZE + MPI_BSEND_OVERHEAD) * 2 * NUM_BSEND_TYPES);
