...
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/umpire_persistent3 \
// RUN: 2>&1 \
// RUN: | %filecheck-may-segfault %s

// CHECK: [MUST-REPORT]{{.*Error: from: call MPI_Waitall@.*0: Argument 2 [(]array_of_requests[)] has to be an array of predefined or user defined requests}}
