/* -*- Mode: C; -*- */
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/umpire_partial-recv \
// RUN: 2>&1 \
...
int main(int argc, char** argv)
{
    ...
    int i;
    int blocklens[3];
    MPI_Aint displs[3], basic_extent;
    ...
    displs[2] =
        ((unsigned long)&(big_struct_buf[0].the_other_double)) - ((unsigned long)big_struct_buf);
    ...
    if (basic_extent != sizeof(test_small_struct_t)) {
        fprintf(stderr, "(%ld): Unexpected extent for small struct\n", basic_extent);
        MPI_Abort(MPI_COMM_WORLD, 666);
    }

    MPI_Type_extent(newtype[1], &basic_extent);
    if (basic_extent != sizeof(test_big_struct_t)) {
        fprintf(stderr, "(%ld): Unexpected extent for big struct\n", basic_extent);
        MPI_Abort(MPI_COMM_WORLD, 666);
    }
