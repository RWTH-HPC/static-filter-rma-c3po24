...
// RUN: %must-bin-dir/umpire_collective-exhaustive-byte-int-mismatch 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*Error: from: call MPI_Bcast@.*[0-9]: Two collective calls cause a type mismatch!}}
...
void complex_prod(void* inp, void* inoutp, int* len, MPI_Datatype* dptr)
{
    int i, stop;
    complex_t c;
...
#ifdef RUN_GATHER
        for (i = 0; i < OP_COUNT; i++)
            MPI_Gather(&buf0[rank * buf_size], buf_size, MPI_INT, buf1, buf_size, MPI_INT, 0, comm);
#endif
...
#ifdef RUN_BAND
        for (i = 0; i < OP_COUNT; i++)
            MPI_Reduce(buf0, buf1, buf_size * sizeof(int), MPI_BYTE, MPI_BAND, 0, comm);
#endif
...
#ifdef RUN_BOR
        for (i = 0; i < OP_COUNT; i++)
            MPI_Reduce(buf0, buf1, buf_size * sizeof(int), MPI_BYTE, MPI_BOR, 0, comm);
#endif
...
#ifdef RUN_BXOR
        for (i = 0; i < OP_COUNT; i++)
            MPI_Reduce(buf0, buf1, buf_size * sizeof(int), MPI_BYTE, MPI_BXOR, 0, comm);
#endif

#ifdef RUN_USEROP
        for (i = 0; i < OP_COUNT; i++)
            MPI_Reduce(a, answer, buf_size * 2 * sizeof(double), MPI_BYTE, user_op, 0, comm);
#endif
#endif
...
#ifdef RUN_BAND
        for (i = 0; i < OP_COUNT; i++)
            MPI_Allreduce(buf0, buf1, buf_size * sizeof(int), MPI_BYTE, MPI_BAND, comm);
#endif
...
#ifdef RUN_BOR
        for (i = 0; i < OP_COUNT; i++)
            MPI_Allreduce(buf0, buf1, buf_size * sizeof(int), MPI_BYTE, MPI_BOR, comm);
#endif
...
#ifdef RUN_BXOR
        for (i = 0; i < OP_COUNT; i++)
            MPI_Allreduce(buf0, buf1, buf_size * sizeof(int), MPI_BYTE, MPI_BXOR, comm);
#endif

#ifdef RUN_USEROP
        for (i = 0; i < OP_COUNT; i++)
            MPI_Allreduce(a, answer, buf_size * 2 * sizeof(double), MPI_BYTE, user_op, comm);
#endif
#endif
...
#ifdef RUN_USEROP
        for (i = 0; i < OP_COUNT; i++)
            MPI_Scan(a, answer, buf_size * 2 * sizeof(double), MPI_BYTE, user_op, comm);
#endif
#endif
    }
