...
// RUN: %must-bin-dir/umpire_no-error4-any_src 2>&1 \
...
int main(int argc, char** argv)
{
    ...
    if (nprocs < 2) {
        ...
        if (rank == 0) {
            ...
            MPI_Recv_init(buf1, buf_size, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &aReq[1]);
            ...
            MPI_Recv_init(buf0, buf_size, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &aReq[0]);
