...
// RUN: %must-bin-dir/umpire_no-error-derived-comms 2>&1 \
...
#define buf_size 128
#define DCOMM_CALL_COUNT                                                                           \
    7 /* MPI_Cart_create; MPI_Cart_sub;
    ...
    if (nprocs < 4) {
        ...
        if (rank % 3) {
            MPI_Intercomm_create(
                ...
                0,
                MPI_COMM_WORLD,
                (((nprocs % 3) == 2) && ((rank % 3) == 2)) ? nprocs - 1
                                                           : nprocs - (rank % 3) - (nprocs % 3),
        ...
        if (DCOMM_CALL_COUNT > 4) {
            ...
            MPI_Graph_create(MPI_COMM_WORLD, GRAPH_SZ, graph_index, graph_edges, 1, &dcomms[4]);
#else
        ...
        for (i = 0; i < DCOMM_CALL_COUNT; i++) {
            if (dnprocs[i] > 1) {
                if (drank[i] == 0) {
                    for (j = 1; j < dnprocs[i]; j++) {
                        MPI_Recv(buf, buf_size, MPI_INT, MPI_ANY_SOURCE, 0, dcomms[i], &status);
                    }
                } else {
                    memset(buf, 1, buf_size);
        ...
        if ((rank % 3) == 1) {
            for (j = 0; j < intersize; j++) {
                MPI_Recv(buf, buf_size, MPI_INT, MPI_ANY_SOURCE, 0, intercomm, &status);
            }
        } else if ((rank % 3) == 2) {
            for (j = 0; j < intersize; j++) {
                memset(buf, 1, buf_size);
