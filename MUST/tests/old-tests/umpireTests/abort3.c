/* -*- Mode: C; -*- */
// RUN: %must-run %mpiexec-numproc-flag 3 %must-bin-dir/umpire_abort3 2>&1 \
// RUN: | %filecheck --implicit-check-not '[MUST-REPORT]{{.*(Error: from: \
// RUN: call MPI_Abort@.*[0,2-9]: Argument 2 [(]errorcode[)] has to be a \
// RUN: non-negative integer, but is negative)}}' %s

// CHECK: [MUST-REPORT]{{.*Error: from: call MPI_Abort@.*1: Argument 2 [(]errorcode[)] has to be a non-negative integer, but is negative}}
