...
// RUN: %must-bin-dir/marmot_c_pers_requests3 2>&1 \
// RUN: | %filecheck %s

// CHECK: [MUST-REPORT]{{.*There are 2 requests that are not freed when MPI_Finalize was issued,}}
 ...
 *
 *  $Id: pers_requests3.c 319 2004-08-16 11:25:02Z rusbetti $  
 */
...
int main(int argc, char** argv)
{
    ...
    if (rank == 1) {
        ...
        MPI_Startall(2, request_arr);
        MPI_Request_free(&request_arr[0]);
