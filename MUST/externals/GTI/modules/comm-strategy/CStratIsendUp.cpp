...
CStratIsendUp::CStratIsendUp(const char* instanceName)
    : ModuleBase<CStratIsendUp, CStratUpQueue>(instanceName), myReceivedUnexpectedMessages()
{
...
// shutdown
//=============================
GTI_RETURN CStratIsendUp::shutdown(GTI_FLUSH_TYPE flush_behavior, GTI_SYNC_TYPE sync_behavior)
{
    ...
    GTI_RETURN (*buf_free_function)(void* free_data, uint64_t num_bytes, void* buf))
{
    ...
    while (!doneMsg1 || !doneMsg2) {
        ...
        while (!myRequests.empty() && (completed || myRequests.size() >= myMaxNumRequests)) {
            protocol->test_msg(myRequests.front().get_request(), &completed, NULL, NULL);
        ...
        if (!doneMsg1) {
            ...
            if (ret == GTI_ERROR) {
                ...
                doneMsg1 = true;
                myRequests.push_back(CStratIsendRequest(
                    temp_buf,
                    sizeof(uint64_t) * 2,
                    NULL,
                    my_buf_free_function,
        ...
        if (doneMsg1 && !doneMsg2) {
            ...
            if (ret == GTI_ERROR) {
                ...
                doneMsg2 = true;
                myRequests.push_back(
                    CStratIsendRequest(buf, num_bytes, free_data, buf_free_function, req2));
            }
        }
    }
    ...
    GTI_RETURN (**out_buf_free_function)(void* free_data, uint64_t num_bytes, void* buf))
{
    ...
    if (!myReceivedUnexpectedMessages.empty()) {
        ...
        if (out_flag)
            *out_flag = 1;
        if (out_num_bytes)
            *out_num_bytes = item.num_bytes;
        if (out_buf)
            *out_buf = item.buf;
        if (out_free_data)
            *out_free_data = item.free_data;
        if (out_buf_free_function)
            *out_buf_free_function = item.buf_free_function;
        return GTI_SUCCESS;
    }
...
// flush
//=============================
GTI_RETURN CStratIsendUp::flush(void)
{
    //==Check whether a connection exists
    if (!protocol->isConnected()) {
        return GTI_ERROR_NOT_INITIALIZED;
