 ...
 */
class CStratThreadedIntra : public ModuleBase<CStratThreadedIntra, CStratIntraQueue>,
                            public CStratThreadedAggregator,
                            public CStratAggregateReceiver
{
  protected:
    ...
    std::map<int, std::list<CStratQueueItem>>
        myReceivedUnexpectedMessages; // maps channel from which we got it to the buffer (either an
                                      // aggregate OR a long message, long messages will have a
                                      // free_data of "(void*)1")
    ...
    /**
     * Internal implementation of flush, offers choice of to block for completion of flushed
     * messages or not.
     */
