 ...
 */
class CStratPRecvDown : public ModuleBase<CStratPRecvDown, CStratDownQueue>,
                        public CStratPRecv,
                        public CStratBufReceiver
{
  protected:
    ...
    std::list<OutstandingBroadcastInfo>
        myOutstandingBroadcasts; /**< List of outstanding broadcasts.*/

    /**
     * Tries to complete broadcasts.
     * If block=false then the function tests until the first broadcast could not be completed and
     * otherwise it cycles until all broadcasts are finished.
     */
