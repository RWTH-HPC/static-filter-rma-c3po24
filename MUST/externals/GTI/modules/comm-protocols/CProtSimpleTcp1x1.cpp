...
CProtSimpleTCP::CProtSimpleTCP(const char* instanceName)
    : ModuleBase<CProtSimpleTCP, I_CommProtocol>(instanceName), myPlaceId(0), myIsShutdown(false),
      myNextRequestID(1)
{
...
GTI_RETURN CProtSimpleTCP::startup(void)
{
    ...
    uint64_t tierSize;
    uint64_t selfID;
    unsigned short int serverPort = 0;
    ...
    assert(ret == static_cast<int>(len));
    ...
    int bytes =
#endif
        ::blocking_recv(serverSocket, connectionInfoBuffer, sizeof(uint64_t));
    ...
    bytes =
#endif
        ::blocking_recv(serverSocket, connectionPartners, len);
    assert(bytes == static_cast<int>(len));
    ...
    for (unsigned int i = 0; i < myNumSockets; i++) {
        ...
        if (commSide == 'b') {
            ...
            bytes =
#endif
                ::all_send(serverSocket, connectionInfoBuffer, sizeof(uint64_t));
            assert(bytes == sizeof(uint64_t));
...
GTI_RETURN CProtSimpleTCP::ssend(void* buf, uint64_t num_bytes, uint64_t channel)
{
    ...
    ssize_t sentBytes =
#endif
        ::all_send(mySockets[channel], buf, num_bytes);
    assert(sentBytes == static_cast<ssize_t>(num_bytes));
    ...
    uint64_t* out_channel)
{
     ...
     *
     * @todo IMPORTANT: This violates order if we mix recv's with irecv's (hopefully we don't in our
     * use case)
     */
#if !defined(NDEBUG)
    ssize_t sentBytes =
#endif
        ::blocking_recv(mySockets[0], out_buf, num_bytes);
    assert(sentBytes == static_cast<ssize_t>(num_bytes));
    ...
    uint64_t* out_channel)
{
    ...
    int retBytes = ::recv(
        ...
        buf,
        4,
        MSG_PEEK | MSG_DONTWAIT); /**< Use a length of 4 bytes just to peek.*/

    if (retBytes > 0) {
        // Bingo we have something to receive
        int rcvBytes = ::blocking_recv(mySockets[0], buf, len);
        assert(rcvBytes <= static_cast<int>(len));
        ...
        if (requestId == request) {
            ...
            myCompletedRequests.insert(std::make_pair(requestId, std::make_pair(0, rcvBytes)));
        }
    } else {
    ...
    unsigned int* request_finished)
{
    ...
    if ((myActiveRequests[channel].empty()) && (myActiveRequests[myNumSockets].empty())) {
        ...
        if (target_buffer == NULL) {
            ...
            sentBytes =
#endif
                ::blocking_recv(mySockets[channel], tempBuffer, size);
            assert(static_cast<size_t>(sentBytes) == size);
            ...
            sentBytes =
#endif
                ::blocking_recv(mySockets[channel], target_buffer, size);
            assert(static_cast<size_t>(sentBytes) == size);
        ...
        sentBytes =
#endif
            ::blocking_recv(mySockets[channel], r.second.second.first, size);
        assert(static_cast<size_t>(sentBytes) == size);
