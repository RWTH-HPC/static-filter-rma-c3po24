...
/**
 * @file ThreadedMpiPlace.cpp
 *       A placement driver that is started as an MPI
 *       application and uses multi-threading to allow
 *       concurrent communication (loggs, mgmt, trace).
 *
 *
...
mGET_INSTANCE_FUNCTION(ThreadedMPIPlace)
mFREE_INSTANCE_FUNCTION(ThreadedMPIPlace)
mPNMPI_REGISTRATIONPOINT_FUNCTION(ThreadedMPIPlace)
...
//=============================
inline void handlePlaceStartup(void)
{
    ...
    ThreadedMPIPlace* place;
    place = ThreadedMPIPlace::getInstance(instanceName);
    assert(place);
    place->init();
    place->run();
    ThreadedMPIPlace::freeInstanceForced(place);
    place = NULL;
}
...
{
    if (ThreadPlaceInitialized)
        return MPI_SUCCESS;

    ThreadPlaceInitialized = 1;
    // init MPI first
...
void ThreadedMPIPlace::init()
{
    ...
    if (static_cast<ssize_t>(subModInstances.size()) > wrapperModuleIndex)
...
// ThreadedMPIPlace
//=============================
ThreadedMPIPlace::ThreadedMPIPlace(const char* instanceName)
    : ModuleBase<ThreadedMPIPlace, I_Place>(instanceName)
{
}
...
// ~ThreadedMPIPlace
//=============================
ThreadedMPIPlace::~ThreadedMPIPlace(void)
{
    if (myTraceRecv) {
...
// testBroadcast
//=============================
GTI_RETURN ThreadedMPIPlace::testBroadcast(void)
{
...
// testIntralayer
//=============================
GTI_RETURN ThreadedMPIPlace::testIntralayer(void)
{
...
// run
//=============================
void ThreadedMPIPlace::run(void)
{
#ifdef GTI_VT
    VT_TRACER("ThreadedMPIPlace::run");
#endif /*GTI_VT*/
    ...
    uint64_t size;
    void *buf, *free_data;
    uint64_t numClients, numChannels;
#ifdef GTI_DEBUG
    uint64_t numOps = 0;
#endif
    ...
    const uint64_t timeoutThreshold = 500000; // .5s
    ...
    myTraceRecv->getNumClients(&numClients);
    numChannels = numClients;
#ifdef GTI_DEBUG
    printf("\n\nCLIENTS: %ld\n\n", numClients);
#endif
...
#ifdef GTI_VT
                VT_TRACER("ThreadedMPIPlace::run::gotQueuedRecord");
#endif /*GTI_VT*/
        ...
        if (enforceTimeout ||
...
#ifdef GTI_VT
            {
                VT_TRACER("ThreadedMPIPlace::run::test");
#endif /*GTI_VT*/
            ...
            if (ret == GTI_SUCCESS) // Necessary as strategy may also return it is not ready yet ...
            {
                ...
                if (flag) {
...
#ifdef GTI_VT
                    VT_TRACER("ThreadedMPIPlace::run::evaluateNewRecord");
#endif /*GTI_VT*/
                    ...
                    if (!tree)
                        tree = new SuspensionBufferTree(
                            channelId->getNumUsedSubIds() - 1,
                            channelId->getSubIdNumChannels(channelId->getNumUsedSubIds() - 1));
...
#ifdef GTI_VT
                        VT_TRACER("ThreadedMPIPlace::run::sleep");
#endif /*GTI_VT*/
...
#ifdef GTI_VT
            VT_TRACER("ThreadedMPIPlace::run::process");
#endif /*GTI_VT*/
            // process
...
//=============================
bool ThreadedMPIPlace::receiveAndProcessIntraLayerEvent(bool* hadEvent)
{
    GTI_RETURN (*free_function)(void* free_data, uint64_t num_bytes, void* buf);
    int flag;
    uint64_t size;
...
//=============================
bool ThreadedMPIPlace::receiveAndProcessBroadcastEvent(bool* hadEvent, bool* pOutFinalize)
{
    GTI_RETURN (*free_function)(void* free_data, uint64_t num_bytes, void* buf);
    int flag = false;
    uint64_t size;
    ...
    for (size_t i = 0; i < myUpStrats.size() && !flag; i++) {
        uint64_t upCommStartTime = 0;
        if (myProfiler)
            upCommStartTime = getUsecTime();
...
//=============================
bool ThreadedMPIPlace::finishIntraCommunication(void)
{
    if (!myIntraRecv)
        return true;
...
// getNodeInLayerId
//=============================
GTI_RETURN ThreadedMPIPlace::getNodeInLayerId(GtiTbonNodeInLayerId* id)
{
...
//=============================
GTI_RETURN ThreadedMPIPlace::getLayerIdForApplicationRank(int rank, GtiTbonNodeInLayerId* id)
{
    return GTI_SUCCESS;
}
/*EOF*/
