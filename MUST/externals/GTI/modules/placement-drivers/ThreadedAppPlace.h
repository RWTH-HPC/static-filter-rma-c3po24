...
#ifndef THREADED_APP_PLACE_H
#define THREADED_APP_PLACE_H

namespace gti
{

/**
 * Helper to ease the startup from CProtSMQueue
 */
extern "C" void handlePlaceStartup(void);
 ...
 */
class ThreadedAppPlace : public ModuleBase<ThreadedAppPlace, I_Place>, public GtiHelper
{
  public:
    /**
     * Constructor.
     ...
     */
    ThreadedAppPlace(const char* instanceName);

    /**
     * Destructor.
     */
    ~ThreadedAppPlace(void);
  ...
  protected:
    I_CommStrategyDown* myTraceRecv{nullptr};
    I_PlaceReceival* myReceival{nullptr};
    I_Module* myWrapper{nullptr};
    I_CommStrategyIntra* myIntraRecv{nullptr};
    I_Profiler* myProfiler{nullptr};
    I_FloodControl* myFloodControl{nullptr};
    std::vector<I_CommStrategyUp*> myUpStrats{};

    uint64_t myIntraCommTime{0}, myIntraCommCount{0};
    uint64_t myUpCommTime{0}, myUpCommCount{0};
    ...
    static void newChannel() { numChannels++; /*numClients++;*/ }
    static uint64_t numClients, numChannels;
}; /*class ThreadedAppPlace*/
} /*namespace gti*/
