...
int main(int argc, char **argv)
{
  ...
  MPI_Session session_A;
  MPI_Session_init(MPI_INFO_NULL, MPI_ERRORS_ABORT, &session_A);

  MPI_Session session_B;
  MPI_Session_init(MPI_INFO_NULL, MPI_ERRORS_ABORT, &session_B);

  MPI_Init_thread(&argc, &argv, MPI_THREAD_SINGLE, NULL);

  {
    MPI_Session session_C;
    MPI_Session_init(MPI_INFO_NULL, MPI_ERRORS_ABORT, &session_C);
    MPI_Session_finalize(&session_C);
  }

  MPI_Finalize();

  MPI_Session_finalize(&session_B);
  MPI_Session_finalize(&session_A);
