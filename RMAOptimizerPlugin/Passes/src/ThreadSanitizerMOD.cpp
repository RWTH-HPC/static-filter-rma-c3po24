...
#define DEBUG_TYPE "tsanMOD"

static cl::opt<bool> ClInstrumentMemoryAccesses(
    "tsanMOD-instrument-memory-accesses", cl::init(true),
    cl::desc("Instrument memory accesses"), cl::Hidden);
static cl::opt<bool>
    ClInstrumentFuncEntryExit("tsanMOD-instrument-func-entry-exit", cl::init(true),
                              cl::desc("Instrument function entry and exit"),
                              cl::Hidden);
static cl::opt<bool> ClHandleCxxExceptions(
    "tsanMOD-handle-cxx-exceptions", cl::init(true),
    cl::desc("Handle C++ exceptions (insert cleanup blocks for unwinding)"),
    cl::Hidden);
static cl::opt<bool> ClInstrumentAtomics("tsanMOD-instrument-atomics",
                                         cl::init(true),
                                         cl::desc("Instrument atomics"),
                                         cl::Hidden);
static cl::opt<bool> ClInstrumentMemIntrinsics(
    "tsanMOD-instrument-memintrinsics", cl::init(true),
    cl::desc("Instrument memintrinsics (memset/memcpy/memmove)"), cl::Hidden);
static cl::opt<bool> ClDistinguishVolatile(
    "tsanMOD-distinguish-volatile", cl::init(false),
    cl::desc("Emit special instrumentation for accesses to volatiles"),
    cl::Hidden);
static cl::opt<bool> ClInstrumentReadBeforeWrite(
    "tsanMOD-instrument-read-before-write", cl::init(false),
    cl::desc("Do not eliminate read instrumentation for read-before-writes"),
    cl::Hidden);
static cl::opt<bool> ClCompoundReadBeforeWrite(
    "tsanMOD-compound-read-before-write", cl::init(false),
    cl::desc("Emit special compound instrumentation for reads-before-writes"),
    cl::Hidden);
static cl::opt<bool> ClUseOptimizerAllowlist(
    "tsanMOD-use-optallowlist", cl::init(false),
    cl::desc("Enable Allowlist from TSanRMAOpt to minimize amount of instrumented Addresses"),
    cl::Hidden);
static cl::opt<bool> ClUseOptimizerRemoteAccessType(
    "tsanMOD-use-optallowlist-remoteaccesstypeext", cl::init(false),
    cl::desc("Utilize absence of remote writes to further improve performance, if possible"),
    cl::Hidden);
static cl::opt<bool> ClUseTSanFilterList(
    "tsanMOD-use-filterlist", cl::init(false),
    cl::desc("Use a filtering list"),
    cl::Hidden);
...
STATISTIC(NumOmittedNotWhitelisted, "Number of accesses ignored due to RMAOptimizerPlugin Whitelist");
STATISTIC(NumOmittedRMAAccessType, "Number of accesses ignored due to RMAOptimizerPlugin Access Type");
...
struct ThreadSanitizer {
  ...
  TSanRMAOptimizerAnalysis::TSanRMAOptResult *TSRMAOpt = NULL;
  TSanInstrFilter::TSanInstrFilterResult *TSFilter = NULL;
...
PreservedAnalyses ThreadSanitizerMODPass::run(Function &F,
                                           FunctionAnalysisManager &FAM) {
  ThreadSanitizer TSan;
  if (ClUseOptimizerAllowlist) {
    TSan.TSRMAOpt = FAM.getResult<ModuleAnalysisManagerFunctionProxy>(F).getCachedResult<TSanRMAOptimizerAnalysis>(*F.getParent());
    if (TSan.TSRMAOpt == NULL) {
      errs() << "Unable to utilize ThreadSanitizer instrumentation allowlist. Expect performance loss" << "\n";
    }
  }
  if (ClUseTSanFilterList) {
    TSan.TSFilter = FAM.getResult<ModuleAnalysisManagerFunctionProxy>(F).getCachedResult<TSanInstrFilter>(*F.getParent());
    if (TSan.TSFilter == NULL) {
      errs() << "Unable to utilize ThreadSanitizer Filtering List!" << "\n";
    }
  }
...
PreservedAnalyses ModuleThreadSanitizerMODPass::run(Module &M,
                                                 ModuleAnalysisManager &MAM) {
  insertModuleCtor(M);
  PreservedAnalyses P;
  if (ClUseOptimizerAllowlist) {
    MAM.getResult<TSanRMAOptimizerAnalysis>(M); // Make sure it runs before entering function instrumentation
    P.preserve<TSanRMAOptimizerAnalysis>();
  }
  if (ClUseTSanFilterList) {
    MAM.getResult<TSanInstrFilter>(M); // Make sure it runs before entering function instrumentation
    P.preserve<TSanInstrFilter>();
  }
  return P;
}
...
void ThreadSanitizer::chooseInstructionsToInstrument(
  ...
  for (Instruction *I : reverse(Local)) {
    ...
    if (TSRMAOpt) {
      TSanRMAOptimizerAnalysis::SharedResource ShAddr = {Addr, TSanRMAOptimizerAnalysis::ShResType::DirtyBuf};
      auto ShResItr = std::find(TSRMAOpt->SharedResources.begin(), TSRMAOpt->SharedResources.end(), ShAddr);
      if (ShResItr == TSRMAOpt->SharedResources.end()) {
        NumOmittedNotWhitelisted++;
        continue;
      }
      if (ClUseOptimizerRemoteAccessType && !IsWrite) {
        TSanRMAOptimizerAnalysis::SharedResource ShRes = *ShResItr;
        if ((TSRMAOpt->RemoteIsReadOnly && ShRes.Type == TSanRMAOptimizerAnalysis::ShResType::RemoteBuf) ||
            (TSRMAOpt->RemoteIsWriteOnly && (ShRes.Type == TSanRMAOptimizerAnalysis::ShResType::WriteBuf || ShRes.Type == TSanRMAOptimizerAnalysis::ShResType::ReadBuf))) {
          NumOmittedRMAAccessType++;
          continue;
        }
      }
    }
...
bool ThreadSanitizer::sanitizeFunction(Function &F,
  ...
  if (TSFilter) {
    // Allowlist provided and current function not in list
    if (TSFilter->allowlist  && TSFilter->FunctionFilterList.find(&F) == TSFilter->FunctionFilterList.end()) return false;
    // Denylist provided and current function is in list
    if (!TSFilter->allowlist && TSFilter->FunctionFilterList.find(&F) != TSFilter->FunctionFilterList.end()) return false;
  }
